...
## 🛠 최근 작업 내역 (2026-02-10)
- **잔액 조회 기능 정상화**: `python-kis` 라이브러리 업데이트에 따른 계좌 정보 접근 방식(`api.account()` 메소드 호출 및 `.account_number.number` 속성 사용) 수정.
- **모의투자 환경 최적화**: 잔액 조회 시 모의투자용 TR ID(`VTTC8434R`)를 적용하여 데이터 연동 오류 해결.
- **응답 처리 개선**: API 응답 데이터(`KisDynamicDict`)에서 출력 필드를 안전하게 추출하도록 로직 보완.
